...
function renderDaySelection(dm) {
    ...
    for (i = dm; i < dm + sum; i++) {
        ...
        btn.className = 'day w-full ' + sum + ' text-center inline-block rounded-lg mr-2 py-1 day-selection ' + cursor;
        btn.type = "button";
...
try {
    ...
    currentMonth2.setFullYear(dateInitial[2],parseInt(dateInitial[0])-1,dateInitial[1])
    
    var selectedDate2 = new Date()
    selectedDate2.setFullYear(dateInitial[2],parseInt(dateInitial[0])-1,dateInitial[1])
...
function getFlights(id,dateSend,begin,to){
    ...
    $.ajax({
        url:`/fligths/get/${date_departure}/${date_return}/${begin}/${to}/${document.getElementById("adults").value}/${document.getElementById("children").value}/${document.getElementById("infants").value}`,
        type:"get",
        dataType:"json",
...
function addRowTableOutboundFlights(table,element) {
    ...
    div.className = 'inline-flex justify-center items-center bg-gradient-to-br from-itravel-900 to-itravel-600  p-0.5 rounded-lg cursor-pointer';
